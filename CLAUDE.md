...
- `src/feedback/` — Consent resolution (`FeedbackConsentResolver`), consent persistence (`FileConsentStore`), and the MCP elicitation prompt used to ask the user once.
...
Required env vars: `UNLEASH_BASE_URL`, `UNLEASH_PAT`. Optional: `UNLEASH_DEFAULT_PROJECT`, `UNLEASH_DEFAULT_ENVIRONMENT`, `LOG_LEVEL`, `APP_LOG_FILE`, `MCP_STDIO_LOG_FILE`, `UNLEASH_MCP_CLIENT_ATTRIBUTION` (set to `off` to disable client attribution in outbound headers; default: enabled), `UNLEASH_FEEDBACK_URL` (base URL of the Unleash instance that receives `send_feedback` reports; defaults to the sandbox instance), `UNLEASH_MCP_SEND_FEEDBACK` (`true` or `false` to grant or deny `send_feedback` consent without a prompt; overrides the consent file). CLI flags: `--dry-run`, `--log-level <level>`. See `.env.example` for reference.
